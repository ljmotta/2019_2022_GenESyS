...
type
    TNewModuleDuplicate = Class(TSourceModule)
    private
      ...
      published
         ...
         {... YOUR CODE HERE ...}
         property Duplicates:string   read aCreations  write aCreations;
      end;

      TModuleManagerDuplicate = class(TObject)
      private
         function  AddModule: TNewModuleDuplicate;
         procedure ReadModule(palavra: TStringList);
         procedure SaveModule(generalModule: TModule; var palavra: TStringList);
         procedure Execute(var thisModule: TNewModuleDuplicate; entidade: word); virtual;
         function  VerifySymbols(blocoGeral: TModule): string;
         procedure UserExecute(var thisModule: TNewModuleDuplicate; entity: word);
         procedure UserRead(var thisModule: TNewModuleDuplicate; words: TStringList);
         procedure UserSave(thisModule: TNewModuleDuplicate; var words: TStringList);
         procedure UserVerifySymbols(thisModule: TNewModuleDuplicate; var verifyList: TStringList);
      public
         constructor Create;
   ...
   var
      PlugInDuplicate : TModuleManagerDuplicate;
{$IFDEF IsDLLPlugin}
...
procedure TNewModuleDuplicate.UserCreate(novoId: word; novoNome, novoKind: string);
begin
   ...
   {... YOUR CODE HERE ...}
   aCreations := '';
   aEntitiesPerCreation := '0'; {no entity is created with simulation startup}
end;

procedure TModuleManagerDuplicate.UserRead(var thisModule:TNewModuleDuplicate; words:TStringList);
begin
    ...
    {... YOUR CODE HERE ...}
    thisModule.aCreations := words.Strings[0];
end;

procedure TModuleManagerDuplicate.UserSave(thisModule:TNewModuleDuplicate; var words: TStringList);
begin
    ...
    {... YOUR CODE HERE ...}
    words.Add(thisModule.aCreations);
end;

procedure TModuleManagerDuplicate.UserVerifySymbols(thisModule:TNewModuleDuplicate; var verifyList:TStringList);
begin
   ...
   {... YOUR CODE HERE ...}
   Genesys.AuxFunctions.VerifySymbol(thisModule.Name, 'Duplicates', thisModule.aCreations, cEXPRESSION, true);
end;

procedure TModuleManagerDuplicate.UserExecute(var thisModule:TNewModuleDuplicate; entity:word);
var numDuplicates, newID: word;
    i: integer;
begin
   ...
   {... YOUR CODE HERE ...}
   numDuplicates := trunc(Genesys.Model.SIMAN.StringEvaluateFormula(thisModule.aCreations));
   for i := 1 to numDuplicates do begin
      newID := Genesys.Model.SIMAN.EntityCreate(thisModule.aEntityTypeID);
      Genesys.Model.TraceSimulation(cTLModuleIntern,ThisModule.ID,entity,'Duplicates as new entity '+IntToStr(newID));
      Genesys.Model.SIMAN.EntitySendToModuleNumber(newID, thisModule.NextID[1], 0);
   end;
   Genesys.Model.SIMAN.EntitySendToModuleNumber(entity, thisModule.NextID[0], 0);
end;
...
constructor TNewModuleDuplicate.Create(novoId: word; novoNome, novoKind: string);
begin
   try
      inherited;
...
procedure TModuleManagerDuplicate.ReadModule(palavra: TStringList);
var thisModule: TNewModuleDuplicate;
    newID: word;
 begin
    try
       newID := Genesys.Model.ModuleAdd(MODULE_KIND);
       thisModule := TNewModuleDuplicate(Genesys.Model.ModuleByID(newID));
...
procedure TModuleManagerDuplicate.SaveModule(generalModule: TModule; var palavra: TStringList);
var thisModule: TNewModuleDuplicate;
begin
   try
       thisModule := TNewModuleDuplicate(generalModule);
...
function TModuleManagerDuplicate.VerifySymbols(blocoGeral: TModule): string;
var listaVerificacoes: TStringList;
    thisModule: TNewModuleDuplicate;
begin
   try
      thisModule := TNewModuleDuplicate(blocoGeral);
      GenesysErrorMessage := '';
...
procedure TModuleManagerDuplicate.Execute(var thisModule: TNewModuleDuplicate; entidade: word);
begin
   try
...
constructor TModuleManagerDuplicate.Create;
begin
   inherited Create;
end;
...
function TModuleManagerDuplicate.AddModule: TNewModuleDuplicate;
var newModule: TNewModuleDuplicate;
begin
   newModule := TNewModuleDuplicate.Create(0,'', MODULE_KIND);
   result := newModule;
end;

procedure TModuleManagerDuplicate.ExportedMethodsAccess(metodo: TPlugInMethod; var umPonteiro1: pointer; var umPonteiro2: pointer);
var auxStr: string;
    auxBool: boolean;
    ptrBloco: ^TModule;
    ptrStrList: ^TStringList;
    umComponente : TNewModuleDuplicate;
    umaEntidade : Word;
begin
   try
   if metodo = pmReadModule  then begin
      ...
      PlugInDuplicate.ReadModule(ptrStrList^);
      end
      ...
      PlugInDuplicate.SaveModule(ptrBloco^, ptrStrList^);
      ...
      ptrBloco := umPonteiro1;
      auxStr := PlugInDuplicate.VerifySymbols(ptrBloco^);
      GetMem(umPonteiro1, SizeOf(auxStr));
      string(umPonteiro1^) := auxStr;
      end
   else if metodo = pmINCLUDEMODULE then begin
      umComponente := PlugInDuplicate.AddModule;
      GetMem(umPonteiro1, SizeOf(umComponente));
      TNewModuleDuplicate(umPonteiro1) := umComponente;
      end
   else if metodo = pmExecute then begin
      umComponente := TNewModuleDuplicate(umPonteiro1^);
      umaEntidade := Word(umPonteiro2^);
      PlugInDuplicate.Execute(umComponente,umaEntidade);
      end
   else begin
...
{$IFDEF IsDllPlugin}
   Genesys := umSimulador;
{$ENDIF}
   exportedMethods := PlugInDuplicate.ExportedMethodsAccess;
end;
...
begin
   PlugInDuplicate := TModuleManagerDuplicate.Create;
end.

finalization
   PlugInDuplicate.Destroy;
