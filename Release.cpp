...
Release::Release(Model* model) : ModelComponent(model) {
	...
	_resource = (Resource*) _model->getInfraManager()->getInfrastructure(Util::TypeOf<Resource>(), "Resource 1");
...
void Release::setQueueName(std::string _queueName) {
	if (_queue->getName() != _queueName) {
		Queue* queueWithNewName = (Queue*) _model->getInfraManager()->getInfrastructure(Util::TypeOf<Queue>(), _queueName);
...
void Release::setResourceName(std::string _resourceName) {
	if (_resource->getName() != _resourceName) {
		Resource* resourceWithNewName = (Resource*) _model->getInfraManager()->getInfrastructure(Util::TypeOf<Resource>(), _resourceName);
