...
void Release::_execute(Entity* entity) {
	...
	_model->traceSimulation(Util::TraceLevel::blockInternal, _model->getSimulatedTime(), entity, this, "Entity frees " + std::to_string(quantity) + " units of resource \"" + resource->getName() + "\"");
	if (_queue->size() > 0) { //  ->getList()->size() > 0) {
		...
		if (firstWaiting->getQuantity() >= quantity) { // entity waiting can seize the resource now
			...
			_model->traceSimulation(Util::TraceLevel::blockInternal, _model->getSimulatedTime(), entity, this, "Entity removes entity " + std::to_string(firstWaiting->getEntity()->getId()) + " from queue \"" + _queue->getName() + "\"");
			_queue->removeElement(firstWaiting, _model->getSimulatedTime()); //->getList()->remove(firstWaiting);
		} else {
			_model->traceSimulation(Util::TraceLevel::blockInternal, _model->getSimulatedTime(), entity, this, "Entity " + std::to_string(firstWaiting->getEntity()->getId()) + " remains waiting in queue \"" + _queue->getName() + "\" for " + std::to_string(firstWaiting->getQuantity()) + " instances of resource \"" + resource->getName() + "\"");
