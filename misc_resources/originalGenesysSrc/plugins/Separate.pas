...
type
    TNewModuleSeparate = Class(TModule)
    private
      ...
      TModuleManagerSeparate = class(TObject)
      private
         function  AddModule: TNewModuleSeparate;
         procedure ReadModule(palavra: TStringList);
         procedure SaveModule(generalModule: TModule; var palavra: TStringList);
         procedure Execute(var thisModule: TNewModuleSeparate; entidade: word); virtual;
         function  VerifySymbols(blocoGeral: TModule): string;
         procedure UserExecute(var thisModule: TNewModuleSeparate; entity: word);
         procedure UserRead(var thisModule: TNewModuleSeparate; words: TStringList);
         procedure UserSave(thisModule: TNewModuleSeparate; var words: TStringList);
         procedure UserVerifySymbols(thisModule: TNewModuleSeparate; var verifyList: TStringList);
      public
         constructor Create;
   ...
   var
      PlugInSeparate : TModuleManagerSeparate;
{$IFDEF IsDLLPlugin}
...
procedure TNewModuleSeparate.UserCreate(novoId: word; novoNome, novoKind: string);
begin
...
procedure TModuleManagerSeparate.UserRead(var thisModule:TNewModuleSeparate; words:TStringList);
begin
...
procedure TModuleManagerSeparate.UserSave(thisModule:TNewModuleSeparate; var words: TStringList);
begin
...
procedure TModuleManagerSeparate.UserVerifySymbols(thisModule:TNewModuleSeparate; var verifyList:TStringList);
begin
...
procedure TModuleManagerSeparate.UserExecute(var thisModule:TNewModuleSeparate; entity:word);
var batchNum, batchedEnt: word;
    atribNum: integer;
begin
   ...
   {... YOUR CODE HERE ...}
   atribNum := Genesys.Model.SIMAN.AttributeIndex('Entity.BatchQueue');
   if atribNum >= 0 then begin
      batchNum := trunc(Genesys.Model.SIMAN.EntityAttribute[entity,atribNum]);
      if batchNum > 0 then begin
         {there is a batch. Retrieves entities from batch}
         while Genesys.Model.SIMAN.QueueNumberOfEntities(batchNum) > 0 do begin
            batchedEnt := Genesys.Model.SIMAN.QueueEntityRank(batchNum,0);
            Genesys.Model.SIMAN.QueueRemoveEntity(batchedEnt,batchNum);
            Genesys.Model.SIMAN.EntitySendToModuleNumber(batchedEnt, thisModule.NextID[0], 0);
         end;
         {clear the batch attribute and queue}
         Genesys.Model.SIMAN.EntityAttribute[entity,atribNum] := 0;
         Genesys.Model.SIMAN.QueueRemove(batchNum);
      end else begin
         Genesys.Model.TraceSimulation(cTLModuleIntern, thisModule.ID, entity, 'No batch to separate');
      end;
   end else begin
      Genesys.Model.TraceSimulation(cTLModuleIntern, thisModule.ID, entity, 'Error: Attribute "Entity.BatchQueue" does not exist')
   end;
   Genesys.Model.SIMAN.EntitySendToModuleNumber(entity, thisModule.NextID[0], 0);
end;
...
constructor TNewModuleSeparate.Create(novoId: word; novoNome, novoKind: string);
begin
   try
      inherited;
...
procedure TModuleManagerSeparate.ReadModule(palavra: TStringList);
var thisModule: TNewModuleSeparate;
    newID: word;
 begin
    try
       newID := Genesys.Model.ModuleAdd(MODULE_KIND);
       thisModule := TNewModuleSeparate(Genesys.Model.ModuleByID(newID));
...
procedure TModuleManagerSeparate.SaveModule(generalModule: TModule; var palavra: TStringList);
var thisModule: TNewModuleSeparate;
begin
   try
       thisModule := TNewModuleSeparate(generalModule);
...
function TModuleManagerSeparate.VerifySymbols(blocoGeral: TModule): string;
var listaVerificacoes: TStringList;
    thisModule: TNewModuleSeparate;
begin
   try
      thisModule := TNewModuleSeparate(blocoGeral);
      GenesysErrorMessage := '';
...
procedure TModuleManagerSeparate.Execute(var thisModule: TNewModuleSeparate; entidade: word);
begin
   try
...
constructor TModuleManagerSeparate.Create;
begin
   inherited Create;
end;
...
function TModuleManagerSeparate.AddModule: TNewModuleSeparate;
var newModule: TNewModuleSeparate;
begin
   newModule := TNewModuleSeparate.Create(0,'', MODULE_KIND);
   result := newModule;
end;

procedure TModuleManagerSeparate.ExportedMethodsAccess(metodo: TPlugInMethod; var umPonteiro1: pointer; var umPonteiro2: pointer);
var auxStr: string;
    auxBool: boolean;
    ptrBloco: ^TModule;
    ptrStrList: ^TStringList;
    umComponente : TNewModuleSeparate;
    umaEntidade : Word;
begin
   try
   if metodo = pmReadModule  then begin
      ...
      PlugInSeparate.ReadModule(ptrStrList^);
      end
      ...
      PlugInSeparate.SaveModule(ptrBloco^, ptrStrList^);
      ...
      ptrBloco := umPonteiro1;
      auxStr := PlugInSeparate.VerifySymbols(ptrBloco^);
      GetMem(umPonteiro1, SizeOf(auxStr));
      string(umPonteiro1^) := auxStr;
      end
   else if metodo = pmINCLUDEMODULE then begin
      umComponente := PlugInSeparate.AddModule;
      GetMem(umPonteiro1, SizeOf(umComponente));
      TNewModuleSeparate(umPonteiro1) := umComponente;
      end
   else if metodo = pmExecute then begin
      umComponente := TNewModuleSeparate(umPonteiro1^);
      umaEntidade := Word(umPonteiro2^);
      PlugInSeparate.Execute(umComponente,umaEntidade);
      end
   else begin
...
{$IFDEF IsDllPlugin}
   Genesys := umSimulador;
{$ENDIF}
   exportedMethods := PlugInSeparate.ExportedMethodsAccess;
end;
...
begin
   PlugInSeparate := TModuleManagerSeparate.Create;
end.

finalization
   PlugInSeparate.Destroy;
