...
Seize::Seize(Model* model) : ModelComponent(model, Util::TypeOf<Seize>()) {
    ...
    /*
    //
    // look for an ELEMENT resource with a default new name
    //    
    std::string defaultResourceName = "Resource " + std::to_string(Util::GenerateNewIdOfType<Resource>());
    _resource = (Resource*) _model->getElementManager()->getElement(Util::TypeOf<Resource>(), defaultResourceName);
    if (_resource == nullptr) { // it does not exist. Simply creates a new resource infra
        ...
        _model->getElementManager()->insertElement(Util::TypeOf<Resource>(), _resource); // inserts the new resource as element
    }
    std::string queueName = _resource->getName() + "_Queue"; // resourceName + "_Queue";
    _queue = (Queue*) _model->getElementManager()->getElement(Util::TypeOf<Queue>(), queueName);
    if (_queue == nullptr) {
        _queue = new Queue(model);
        _queue->setName(_name + "_Queue");
        _model->getElementManager()->insertElement(Util::TypeOf<Queue>(), _queue);
    }
     */
}
...
void Seize::setQueueName(std::string queueName) throw () {
    ...
    if (_queue->getName() != _queueName) {
        Queue* queueNewName = (Queue*) _model->getElementManager()->getElement(Util::TypeOf<Queue>(), _queueName);
        if (queueNewName == nullptr) { // there is no queue with the new name
            ...
            if (!_queue->isLinked()) { // no one else uses it. It can be removed
                _model->getElementManager()->removeElement(Util::TypeOf<Queue>(), _queue);
                //_queue->~Queue();
...
void Seize::setResourceName(std::string resourceName) throw () {
    ...
    if (_resource->getName() != _resourceName) {
        Resource* resourceWithTheNewName = (Resource*) _model->getElementManager()->getElement(Util::TypeOf<Resource>(), _resourceName);
        if (resourceWithTheNewName == nullptr) { // there is no resource with the new name
            ...
            if (!_resource->isLinked()) { // no one else uses it. It can be removed
                _model->getElementManager()->removeElement(Util::TypeOf<Resource>(), _resource);
                //_resource->~Resource();
