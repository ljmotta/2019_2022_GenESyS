...
void MapKernelAndUI::map(ModelElement* elem, QGraphicsItem* uiObj) {
...
void MapKernelAndUI::unmap(ModelElement* elem, QGraphicsItem* uiObj) {
	std::map<ModelElement*, QGraphicsItem*>::iterator it = _mapElemQObj.find(elem);
	if (it != _mapElemQObj.end())
		_mapElemQObj.erase(it);
	std::map<QGraphicsItem*, ModelElement*>::iterator it2 = _mapQObjElem.find(uiObj);
...
QGraphicsItem* MapKernelAndUI::getUI(ModelElement* elem) {
	std::map<ModelElement*, QGraphicsItem*>::iterator it = _mapElemQObj.find(elem);
	if (it != _mapElemQObj.end())
		return (*it).second;
	return nullptr;
...
ModelElement* MapKernelAndUI::getElem(QGraphicsItem* uiObj) {
	std::map<QGraphicsItem*, ModelElement*>::iterator it = _mapQObjElem.find(uiObj);
